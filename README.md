...
- `-rbc`, `--rbc` — режим для PDF с оформлением RBC.
...
- `python converter_PDF_to_CHORDPRO.py --rbc`

### Что делает режим RBC

- Считает, что на входе уже стандартная нотация (`B`, `Bb`) и оставляет её без преобразования.
- Распознаёт `Вступление:` как `Intro`.
- Распознаёт `Проигрыш:` как `Instr.`.
- Нормализует `Куплет N:` в метку куплета `{start_of_verse: N.}`.
- Нормализует заголовок `Припев:` в сокращённый вид `Пр.:`.

Без флага `--rbc` ожидается германская нотация на входе, и она приводится к стандартной (`H -> B`, `B -> Bb`).
