...
class PdfToChordProConverter:
    ...
    def _convert_lines_to_chordpro(self, lines, metadata, filename):
        ...
        while i < len(lines):
            ...
            if i in processed_marker_indices:
                ...
                # But we still need to add it to lines if it contains text we want (like "1. Text")
                # Wait, if we added it via lookahead logic? No, lookahead logic opens the section but adds the CHORDS line.
                # The MARKER line (i) is what we are on now.
                # We need to add it to the current section.
                current_section_lines.append(line)
                i += 1
                continue

            # 1. Identify if a NEW section starts here
            # Markers on current line
            new_section_type, new_label = self._classify_section_start(text)
            
            # Rule 18: Marker might be on NEXT line (if current is chords)
            # Only check if current line itself is NOT a marker
            if not new_section_type and line['is_chord_line'] and i + 1 < len(lines):
                next_text = lines[i+1]['text'].strip()
                ns_type, ns_label = self._classify_section_start(next_text)
                if ns_type:
                    new_section_type = ns_type
                    new_label = ns_label
    ...
    def _classify_section_start(self, text):
        ...
        if text.startswith("Intro") or text.startswith("Вступление"): return ('grid', "Intro")
        ...
        if "Пр" in text or "Chorus" in text:
            # Check if it has colon
            return ('chorus', text.split(':')[0] + ":" if ":" in text else text)
        
        if "Пре-припев" in text or "Пре-пр" in text:
             return ('chorus', text.split(':')[0] + ":" if ":" in text else text)
